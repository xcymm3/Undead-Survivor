...
const initialState: GameSnapshot = { health: 100, hurt: false, pointerLocked: false, weaponsReady: false, weaponIndex: 0, requestedWeapon: 0, switching: false, reloadQueued: false, inventory: WEAPONS.map(gun => gun.capacity), phase: 'ready', mode: 'practice', difficulty: FIXED_DIFFICULTY, survived: 0, alive: 4, zombieCounts: { normal: 4, cone: 0, bucket: 0 }, nearest: null, spawnRate: 0, speed: 0, result: null, ammo: 30, reloading: false, shots: 0, hits: 0, kills: 0, fps: 0, yaw: 0, pitch: 0, sound: true, volume: 1, breach: null, pixelated: false };
  ...
  return <main className={`game-shell phase-${state.phase}`}>
    ...
    {state.phase === 'ready' && <section className="intro mode-menu" aria-labelledby="game-title">
      <div className="intro-copy">
        ...
        <p className="intro-description">森林边缘有了动静。<br />北侧与东侧来袭，六种枪械随时切换。</p>
        <div className="intro-controls"><span><kbd>WASD</kbd> 移动</span><span><kbd>鼠标</kbd> 转向</span><span><kbd>左键</kbd> 开火</span><span><kbd>R</kbd> 换弹</span><span><kbd>1–6 / 滚轮</kbd> 切枪</span></div>
      </div>
    ...
    {(state.phase === 'playing' || state.phase === 'paused') && <div className="hud" aria-label="游戏状态">
      <aside className="objective"><span className="label">{state.mode === 'practice' ? 'FIELD TRAINING' : `SURVIVAL / ${DIFFICULTIES[state.difficulty].label}`}</span><h2>{state.mode === 'practice' ? '僵尸练习靶场' : '在封锁区生存'}</h2><p><span className="tiny-square" /> {state.mode === 'practice' ? '僵尸静止站位，击倒后复位' : '北 / 东侧固定入口 · 移动躲避攻击'}</p><div className="objective-score"><span><b>{String(state.kills).padStart(2, '0')}</b> 击杀</span><span><b>{state.hits}</b> 命中</span><span><b>{state.shots ? Math.round(state.hits / state.shots * 100) : '—'}{state.shots > 0 && '%'}</b> 命中率</span></div></aside>
      {state.mode === 'survival' && <><div className="survival-clock"><span>坚守时长</span><strong data-testid="survival-clock">{formatDuration(state.survived)}</strong></div><aside className="horde-status"><span className="label">INCOMING HORDE</span><p><b>{state.alive}</b> 只僵尸正在逼近</p><small>普通 {state.zombieCounts.normal} · 路障 {state.zombieCounts.cone} · 铁桶 {state.zombieCounts.bucket}</small><small>刷新 {state.spawnRate.toFixed(1)} / 秒 · 移速 {state.speed.toFixed(1)} m/s</small></aside><div className={`proximity ${state.nearest !== null && state.nearest < 4 ? 'danger' : ''}`}>{state.nearest === null ? '留意北侧和东侧入口' : state.nearest < 4 ? '僵尸近身！移动躲避攻击' : '尸群正在接近'}</div></>}
      ...
      <footer className="play-footer"><div><span className="signal-dot" /><span>{state.fps} FPS</span><span className="footer-divider" /><span>自由视角 · 44 × 62 m</span></div><div><span><kbd>WASD</kbd> 移动</span><span><kbd>鼠标</kbd> 转向</span><span><kbd>左键</kbd> {weapon.automatic ? '按住连发' : '单次射击'}</span><span><kbd>ESC</kbd> 暂停</span></div></footer>
    </div>}
    ...
    <dialog ref={dialog} className="settings-dialog" aria-labelledby="settings-title" onCancel={event => { event.preventDefault(); event.stopPropagation(); closeSettings(); }} onKeyDown={event => { if (event.key === 'Escape') event.stopPropagation(); }}>
      ...
      <div className="view-limits"><Icon name="aim" /><p>水平 360°<span>垂直 ±85°</span><small>WASD 移动，点击场景捕获鼠标，ESC 释放并暂停。</small></p></div>
