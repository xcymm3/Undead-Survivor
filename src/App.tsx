...
const initialState: GameSnapshot = { wave: 1, wavesCleared: 0, waveTotal: 9, waveSpawned: 0, intermission: 0, grounded: true, playerHeight: 0, health: 100, hurt: false, pointerLocked: false, weaponsReady: false, weaponIndex: 0, requestedWeapon: 0, switching: false, reloadQueued: false, inventory: WEAPONS.map(gun => gun.capacity), phase: 'ready', mode: 'practice', difficulty: FIXED_DIFFICULTY, survived: 0, alive: 4, zombieCounts: { normal: 4, cone: 0, bucket: 0, imp: 0, shield: 0, berserker: 0, giant: 0, football: 0 }, nearest: null, spawnRate: 0, speed: 0, result: null, ammo: 30, reloading: false, reloadStage: null, shots: 0, hits: 0, kills: 0, fps: 0, yaw: 0, pitch: 0, sound: true, volume: 1, sensitivity: 100, breach: null, pixelated: false, graphicsPreset: 'quality', graphics: { ...DEFAULT_GRAPHICS_SETTINGS }, renderResolution: { width: 1, height: 1, scale: 1, gpu: '未识别' } };
...
export function App() {
  ...
  return <main className={`game-shell phase-${state.phase}`}>
    ...
    {(state.phase === 'playing' || state.phase === 'paused') && <div className="hud" aria-label="游戏状态">
      ...
      <div className={`ammo-panel ${state.ammo === 0 ? 'empty' : ''}`}><div className="weapon-label"><RifleIcon /><span data-testid="weapon-name">{weapon.label}<small>{weapon.short} · {weapon.automatic ? '按住连发' : '单次射击'}</small></span></div><div className="ammo-count"><strong data-testid="ammo">{String(state.ammo).padStart(2, '0')}</strong><span>/ {weapon.capacity}<small>哨站备弹 ∞</small></span></div><div className="ammo-bars" aria-hidden="true">{Array.from({ length: weapon.capacity }, (_, i) => <i key={i} className={i < state.ammo ? 'loaded' : ''} />)}</div><span className="reload-hint">{state.coop?.spectating ? state.reloading ? '队友正在装填…' : '第一人称观战队友' : state.switching ? '切换中…' : pendingWeapon ? `动作结束后切换 · ${WEAPONS[state.requestedWeapon].label}` : state.reloadQueued ? '准备装填…' : state.reloading ? weapon.shellReload ? '逐发装填中…' : '正在更换弹匣…' : state.ammo === 0 ? '弹匣已空 · 按 R 换弹' : <><kbd>R</kbd> 换弹</>}</span></div>
      <div className="weapon-slots" role="group" aria-label="切换武器">{WEAPONS.map((gun, index) => <button key={gun.id} disabled={state.phase !== 'playing' || state.coop?.spectating} aria-label={`切换到${gun.label}`} aria-pressed={index === state.weaponIndex} data-pending={pendingWeapon && index === state.requestedWeapon} onClick={() => game.current?.switchWeapon(index)} title={`${index + 1} · ${gun.label}`}><kbd>{index + 1}</kbd><span>{gun.short}</span><small>{state.inventory[index]}</small></button>)}<p>{state.coop?.spectating ? '正在同步队友武器状态' : '数字键 1–6 / 滚轮切换'}</p></div>
