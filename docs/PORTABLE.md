...
当前成品：`release/Undead Survivor-0.6.0.exe`，Windows 10/11 x64。包含原有自由移动、跳河、波次生存与排行榜，以及 Steam 双人合作模式。0.5.0 是旧单人版本。

## 使用与分发

将 EXE 放在可写入的文件夹，双击启动。无需 Node.js、Chrome 或本地服务器，首次启动会自动解压内置运行环境到临时目录。单人模式可离线游玩；多人模式需要双方分别登录不同的 Steam 账号。

联机的准备、创建/搜索/加入房间、操作与问题排查请阅读 [EXE 联机说明](MULTIPLAYER.md)。发给朋友时附上 `release/联机说明-0.6.0.txt` 即可，不需要源码或 `win-unpacked`。
...
`electron-builder.cjs` 生成 Windows x64 单文件便携版，不自动发布。Steamworks 和 Koffi 原生模块随包分发，并解包到真实文件路径以加载 Steam DLL。EXE 位于 `release/`，二进制不提交到 Git。
...
当前没有第二台电脑及第二个 Steam 账号，尚未完成实际 Spacewar 双机联网验收。成品测试的本机 Steam 状态与具体结果记录在测试目录的 `result.json` 中。
