...
## Genre and theme
Bloom / atmospheric。暖白纸面、陶土色操作按钮。首页以实时游戏画面为背景，右侧操作面板使用 Bloom；设置、排行榜、暂停和多人房间继续使用暖白界面。战斗 HUD 和瞄准镜继续使用原有深色主题，以保证复杂场景上的可读性。

## Structure
- 主界面：实时游戏画面铺底，左侧标题、右侧 Bloom 操作面板；宽度不足 650px 时面板落在标题下方靠右。浮动工具栏位于左上，无营销页脚。
...
只保留操作名、模式区别、落水失败、等待条件、错误和实时状态。移除重复双语副标题与自动保存解释。圆角主按钮，选中状态明确；焦点即时显示；禁用、加载、错误和复制成功分别有可见反馈。无自动播放动画；减少动态效果偏好下停用动效。

## Variants
Bloom 仅覆盖菜单层；不改变游戏场景渲染。窄屏 320 / 375 / 414 / 768 保留可滚动对话框与可达操作。
