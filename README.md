...
- **正式模式**：首波 **9 只**；第 1～6、7～8、9～11、12 波以后每波分别增加 **6、5、4、3 只**。基础移速从 **1.4 米/秒**每波增加 **0.15 米/秒**，达到 **2.5 米/秒**后封顶。刷新率每波增加 **0.1 只/秒**，最高 **2 只/秒**。整波刷完并全部击杀后全员恢复至 **100 HP**，休整 **5 秒**再进入下一波。
- **四阶尸群**：前三波只有一阶；第 4～6 波为一阶 92%、二阶 8%；第 7 波起逐波平滑提高高阶概率，第 10 波开始出现四阶，第 10～12 波每波至少有 1 只橄榄球僵尸，至第 12 波稳定为一阶 60、二阶 28、三阶 10、四阶 3 的相对权重。
...
- **辨认威胁**：小鬼体型小且攻击频繁；持盾者正面防御但攻击时会放低盾牌；狂暴者半血后永久狂暴；巨人缓慢推进并范围砸击；橄榄球僵尸能够冲锋，遇到河道会停在岸前眩晕。路障和铁桶的护具脱落后变成普通僵尸。
...
| 泵动霰弹枪 | 6 | 15 × 15 | 0.64 秒 | 每发 0.55 秒 | 横向扇面爆发 |
| 栓动狙击枪 | 5 | 200 | 0.80 秒 | 1.65 秒 | 远距离重击 |
| 消防斧 | ∞ | 160 | 0.68 秒 | 无 | 2.5 米扇形挥砍 |
| 喷火枪 | 90 | 14 | 0.08 秒 | 2.60 秒 | 15 米多目标穿透 |
| 自动霰弹枪 | 12 | 10 × 17 | 0.30 秒 | 2.25 秒 | 宽扇面连续清群 |
| 重机枪 | 120 | 38 | 0.09 秒 | 3.80 秒 | 长弹链持续火力 |

两把霰弹枪使用中心弹丸加上下两层横向扇面分布，每颗弹丸独立命中，适合瞄准并排僵尸的胸口扫群。在垂直于中心弹道、距枪口 5 米的平面上，单喷弹幕宽约 2.8 米、高约 0.75 米；连喷宽约 3.4 米、高约 0.95 米。右键瞄准不会收窄弹幕，越远单个目标能接到的弹丸越少。弹丸仍由枪口发出并受敌人、墙体及地形遮挡，不会自动锁定或无条件伤害扇面内所有敌人。单喷全中躯干总伤害为 225（原 224），连喷为 170（原 160）；射速、容量与装填不变，完整分级平衡尚未应用。
...
每名玩家需要一台 Windows 电脑和一个不同的 Steam 账号，所有人使用同一版本的 EXE，通过 Spacewar App ID 480 测试。从 Release 下载 `Undead-Survivor-0.7.6.exe` 和 `Multiplayer-Guide-0.7.6.txt` 发给朋友即可，无需安装开发环境。房间流程、画质调整和常见问题见 [EXE 联机说明](docs/MULTIPLAYER.md)。
