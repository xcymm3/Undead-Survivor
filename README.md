...
- **四阶尸群**：前三波只有一阶；第 4～6 波为一阶 92%、二阶 8%；第 7 波起逐波平滑提高高阶概率，第 10 波开始可能出现四阶，至第 12 波稳定为一阶 60、二阶 28、三阶 10、四阶 3 的相对权重。
...
按住右键会用约 0.25 秒把当前武器平滑抬至准星方向，步枪、狙击枪、连喷和重机枪的枪管或机械瞄具随之对齐。该动作不缩放视野，也不改变射线起点、散布、伤害、射速或联机数据，只影响本机第一人称武器模型。
...
每名玩家需要一台 Windows 电脑和一个不同的 Steam 账号，所有人使用同一版本的 EXE，通过 Spacewar App ID 480 测试。从 Release 下载 `Undead-Survivor-0.7.5.exe` 和 `Multiplayer-Guide-0.7.5.txt` 发给朋友即可，无需安装开发环境。房间流程、画质调整和常见问题见 [EXE 联机说明](docs/MULTIPLAYER.md)。
